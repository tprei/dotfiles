...
- **Large repos:** fan out parallel implementation subagents partitioned by directory/area. Spawn each with an explicit `name` and rely on the subagent's own TOML for model selection (per the agent routing rules). Instruct every subagent to fix the underlying error — never re-suppress, never widen an ignore — and to stop and hand back on architectural ambiguity rather than resolving it inline.
