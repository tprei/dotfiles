...
- Treat documentation as a first-class deliverable. 128 session(s) explicitly requested docs or READMEs.
...
- Keep commits scoped and narrated. 284 session(s) asked for clearer commits or change descriptions.
...
- Highlight built-in tools before the user prompts you. 45 session(s) reminded Codex to employ local tooling.
...
- Narrate trade-offs and residual risks instead of silently choosing a path. 44 session(s) asked for clearer communication.
