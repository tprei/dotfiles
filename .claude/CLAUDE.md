...
- Treat documentation as a first-class deliverable. 95 session(s) explicitly requested docs or READMEs.
...
- Default to running or adding tests whenever behavior shifts. 88 session(s) demanded test coverage or reruns.
...
- Keep commits scoped and narrated. 253 session(s) asked for clearer commits or change descriptions.
