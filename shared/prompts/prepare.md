...
If the command was invoked with extra text, treat that text as the scope description. Otherwise ask: "Briefly describe the scope of this bundle (e.g., 'fixing login bug', 'adding user settings'):"
...
Run the current tool's repomix pack-codebase flow with `compress=true` at repository root.
...
Output to a scratch directory outside the repo.
...
"For these entry points, list domain neighbors and shared utilities that are relevant for the requested scope. Exclude test files and changelogs."

### Pass 4: Present candidates
Display findings with scope context:

```
Bundle scope: <scope description>
...
Report: "Bundle attached for scope: <scope description> ({N} files)"
