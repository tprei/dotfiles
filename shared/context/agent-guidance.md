...
- Always look at content as a whole, never take code out of context. Build focused context with `git diff`, `git ls-files`, repo search, and targeted file reads, and gather exactly the files you need before planning or editing. If the user already supplies focused file context, work from that instead of re-deriving it.
- Use official documentation and primary sources for documentation lookup.
- Always write `.md` plan and todo files to a separate folder. If `docs/` already exists, write there; otherwise create one. Timestamp filenames with the date, like `docs/YYYY-MM-DD-<topic>.md`, and follow the conventions of the repo's existing `docs/` folder.
...
- Where the harness provides native search and file-read tools, use those. Only when it provides none, fall back to the shell and prefer `rg` over `grep`, `fd` over `find`, and `bat` over `cat` for previews.
- Shared skills live under `shared/skills/`, and the tool-specific skill directories are symlinks into that canonical library.
...
- Prefer non-interactive command invocations.
