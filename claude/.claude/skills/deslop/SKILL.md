...
model: opus
allowed-tools: Agent Read Glob Grep Edit MultiEdit Write Bash(git:*) Bash(rg:*) Bash(npm:*) Bash(npx:*) Bash(pnpm:*) Bash(yarn:*) Bash(python:*) Bash(python3:*) Bash(uv:*) Bash(poetry:*) Bash(mypy:*) Bash(pyright:*) Bash(ruff:*) Bash(eslint:*) Bash(tsc:*) TaskCreate TaskUpdate TaskList TaskGet
user-invocable: true
...
Run every pattern in `references/slop-catalog.md` with the Grep tool (it uses ripgrep). Produce a per-category, per-file inventory with counts. Don't fix yet — inventory first.
...
- **Large repos:** fan out parallel implementation subagents partitioned by directory/area. Spawn each via the Agent tool with `subagent_type` for an implementation agent and `model: sonnet` (per the agent routing rules). Track each with `TaskCreate`. Instruct every subagent to fix the underlying error — never re-suppress, never widen an ignore — and to stop and return on architectural ambiguity rather than resolving it inline.
...
Lint clean, typecheck reports zero errors, tests pass. Re-run the catalog scan and confirm no new suppressions appeared and the inventory shrank (ideally to zero). Then run the `enemy` agent on non-trivial fixes per the standing reminder, and hand off to `git-commit-specialist`.
