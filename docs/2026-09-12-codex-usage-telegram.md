...
- `plain_image_text(html_text: str) -> str` and `image_line_color(line: str) -> str`: strip the formatted HTML report into fixed-width plain image text that keeps the Unicode bars, percentages, reset times, and padded label columns, and classify healthy, use-soon, warning, exhausted, unknown, and non-row lines for image colors. A non-warning limit with available quota and a reset within one hour uses the gold use-soon color.
