...
- `plain_image_text(html_text: str) -> str`: strip the formatted HTML report into fixed-width plain image text that keeps the Unicode bars, percentages, reset times, and padded label columns, so the image mirrors the grouped message layout.
- `render_image(text: str) -> bytes`: render the plain text to PNG bytes by running the ffmpeg binary resolved from `FFMPEG_BIN` (default `ffmpeg` on `PATH`; the service pins `/usr/bin/ffmpeg`) with the drawtext filter over the fixed-width font resolved from `OMP_USAGE_FONT` (default `/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf`); raise exit code 1 when ffmpeg is missing and exit code 2 when rendering times out or ffmpeg exits non-zero.
