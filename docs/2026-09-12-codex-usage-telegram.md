...
- `render_image(text: str) -> bytes`: render the plain text to PNG bytes by running the ffmpeg binary resolved from `FFMPEG_BIN` (default `ffmpeg` on `PATH`; the service pins `/usr/bin/ffmpeg`) with one drawtext filter per non-empty line, colored status rows, colored provider initial badges, and the fixed-width font resolved from `OMP_USAGE_FONT` (default `/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf`); raise exit code 1 when ffmpeg is missing and exit code 2 when rendering times out or ffmpeg exits non-zero.
