...
- Always produce an explicit plan and confirm scope before editing. 14 session(s) asked for more upfront reasoning.
...
- Treat documentation as a first-class deliverable. 27 session(s) explicitly requested docs or READMEs.
...
- Default to running or adding tests whenever behavior shifts. 23 session(s) demanded test coverage or reruns.
...
- Keep commits scoped and narrated. 24 session(s) asked for clearer commits or change descriptions.
...
- Highlight built-in tools before the user prompts you. 19 session(s) reminded Codex to employ local tooling.
...
- Narrate trade-offs and residual risks instead of silently choosing a path. 12 session(s) asked for clearer communication.
